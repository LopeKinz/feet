...
function routeHeight(index, t) {
  const smoothA = Math.sin((index + t) * 0.38) * 40;
  const smoothB = Math.sin((index + t) * 0.13 + 1.8) * 24;
  return floorY() - 120 + smoothA + smoothB;
}
...
function createChunk(index) {
  ...
  // Main route: continuous and readable traversal instead of fully random scatter.
  const steps = 4;
  const sectionWidth = CHUNK_WIDTH / steps;

  for (let i = 0; i < steps; i++) {
    const seed = index * 31 + i;
    const x = startX + i * sectionWidth + 14;
    const w = sectionWidth - 28;
    const y = routeHeight(index, i * 0.28) + Math.floor(rand(seed) * 18 - 9);
    platforms.push({ x, y, w, h: 16, biome, lane: 'main' });

    // Optional assist platform above the main route for variety.
    if (rand(seed + 7) > 0.45) {
      const aw = 70 + Math.floor(rand(seed + 8) * 45);
      const ax = x + 12 + Math.floor(rand(seed + 9) * Math.max(10, w - aw - 16));
      const ay = y - 90 - Math.floor(rand(seed + 10) * 40);
      platforms.push({ x: ax, y: ay, w: aw, h: 14, biome, lane: 'assist' });
    }
  }

  // Occasional jump challenge but still aligned with the route.
  if (rand(index * 47 + 3) > 0.55) {
    const cx = startX + CHUNK_WIDTH * 0.45;
    const cy = routeHeight(index, 0.52) - 130;
    platforms.push({ x: cx, y: cy, w: 82, h: 14, biome, lane: 'challenge' });
  }

  const coins = [];
  for (const p of platforms) {
    if (p.lane === 'main' && rand(p.x * 0.01 + p.y) > 0.25) {
      coins.push({ x: p.x + p.w * 0.5, y: p.y - 22, taken: false });
    }
  }
